...
SCENARIO("Algo TestMinusProxNoise", " [algo testminusproxnoise]") {
  ...
  ProxNoiseROISizeMm proxNoiseDefaultSizes = {
      .TLLength = 1.3, .noiseBeforeTL = 1., .noiseAfterTL = 1.};
  ...
  GIVEN("cropped 301 F060.png") {
    ...
    std::string filePath =
        getResourcePath("monoplex/algos/resources/cropped 301 F060.png");
    const cv::Mat image =
        cv::imread(filePath, cv::IMREAD_ANYDEPTH | cv::IMREAD_ANYCOLOR);

    WHEN("ProcessorConfig is correct") {

      ProcessorConfig processorConfig(image, false, Red, 2, 6, 1,
                                      ratioNumberPixels301, 1, 2.3);
      AlgoTestMinusProxNoise algo;

      ProfileData profileDataRaw =
          algo.computeProfileForAnalyze(processorConfig, false);
      ProfileData profileDataConvol =
          algo.computeProfileForAnalyze(processorConfig, true);

      double resultRaw =
          algo.compute(processorConfig, profileDataRaw, proxNoiseDefaultSizes);
      double resultConvol = algo.compute(processorConfig, profileDataConvol,
                                         proxNoiseDefaultSizes);
  ...
  GIVEN("cropped 302 F060.png") {
    ...
    std::string filePath =
        getResourcePath("monoplex/algos/resources/cropped 302 F060.png");
    const cv::Mat image =
        cv::imread(filePath, cv::IMREAD_ANYDEPTH | cv::IMREAD_ANYCOLOR);
  ...
  GIVEN("cropped 304 F060.png") {
    ...
    std::string filePath =
        getResourcePath("monoplex/algos/resources/cropped 304 F060.png");
    const cv::Mat image =
        cv::imread(filePath, cv::IMREAD_ANYDEPTH | cv::IMREAD_ANYCOLOR);
  ...
  GIVEN("cropped 305 F060.png") {
    ...
    std::string filePath =
        getResourcePath("monoplex/algos/resources/cropped 305 F060.png");
    const cv::Mat image =
        cv::imread(filePath, cv::IMREAD_ANYDEPTH | cv::IMREAD_ANYCOLOR);
