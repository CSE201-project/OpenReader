function createMultiplexLoopSummary(allResultsFilePath, allResultingValues, paramList, paramName, maxLoop, paramStartValue, paramValueIncrement, rawImagesPath, studyName, analysisDateTime, nbSpotType)
 
    % Determine the Excel model file to get 
    templateExcelFile='AnalysisLoopMultiplexSummary.xlsx';
    ...
    excelFilename=strcat('AnalysisLoopMultiplexSummary_',studyName,'_',paramName,'_',analysisDateTime,'.xlsx');
    ...
    matInfo = [];
    [nbLoopParams,~] = size(paramList);
    for i=1:nbLoopParams
    ...
    for i=1:maxLoop
        ...
        for j=1:6
            if (j < nbSpotType)
                spotTypeOffset = (13 * (j-1));
                testline3s = resultingValues{6 + spotTypeOffset, 5};
                if ismissing(testline3s) || testline3s == 0
                    ...
                    usefulCell = resultingValues(1 + spotTypeOffset:nbAlgos + spotTypeOffset, 1:4);
    ...
    writecell(allLods,filenameExcel,'Sheet','Summary','Range','V32','UseExcel',true);
